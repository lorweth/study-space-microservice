            ...
            try {
                while(!closed.get()) {
                    ...
                    for (ConsumerRecord<String, String> consumerRecord : records) {
                        log.info("Consumed message in topic: {} with value: {}", consumerRecord.topic(), consumerRecord.value());
                        switch (consumerRecord.topic()) {
                            case CREATE_TOPIC:
                                GroupMemberDTO groupMemberDTO = objectMapper.readValue(consumerRecord.value(), GroupMemberDTO.class);
                                groupMemberDTO = groupMemberService.save(groupMemberDTO);
                                log.debug("save groupMember successfully: {}", groupMemberDTO);
                                break;
                            case DELETE_TOPIC:
                                Long groupMemberId = objectMapper.readValue(consumerRecord.value(), Long.class);
