...
class ExamStoreKafkaResourceIT {
    ...
    void setup() {
        ...
        ExamStoreKafkaResource kafkaResource = new ExamStoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/exam-store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/exam-store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
