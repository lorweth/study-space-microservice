...
class GroupStoreKafkaResourceIT {
    ...
    void setup() {
        ...
        GroupStoreKafkaResource kafkaResource = new GroupStoreKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/group-store-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/group-store-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
